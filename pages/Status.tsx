...
export default function Status() {
  ...
  useEffect(() => {
    ...
    const fetchOrderStatus = async () => {
      try {
        ...
        retryCount++;
        if (retryCount > 10) {
          setError('Synchronizing with distributed node taking longer than expected. Please wait...');
        }
        console.warn('Status fetch retry:', retryCount);
      }
    };

    fetchOrderStatus();
    const interval = setInterval(fetchOrderStatus, 3500);
    ...
    return (
      <div className="max-w-4xl mx-auto px-6 py-20 text-center animate-in fade-in duration-700">
        <div className="p-10 bg-blue-500/5 border border-blue-500/10 rounded-[2.5rem] shadow-2xl">
           ...
           <Link to="/swap" className="mt-8 inline-block px-8 py-3 bg-card border border-border text-blue-500 font-black uppercase tracking-widest text-[10px] rounded-xl hover:bg-muted transition-all">
             Cancel Session
           </Link>
        </div>
      </div>
  ...
  if (!order) return (
    <div className="p-24 text-center space-y-4">
      ...
      <div className="text-muted-foreground uppercase font-black text-[10px] tracking-[0.4em] animate-pulse">Syncing with Distributed Ledger...</div>
    </div>
  );
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12 animate-in fade-in slide-in-from-bottom-4 duration-1000">
        ...
        <div className="bg-blue-600/5 p-6 sm:p-10 border-b border-border flex flex-col sm:flex-row items-center justify-between gap-6">
          <div className="text-center sm:text-left">
            <div className="flex items-center justify-center sm:justify-start gap-3 mb-3">
              ...
              <button onClick={() => navigator.clipboard.writeText(id || '')} className="px-3 py-1 bg-blue-600/10 text-blue-500 font-mono text-xs rounded-lg border border-blue-500/10 tracking-widest uppercase hover:bg-blue-600/20 transition-all">
                {id}
              </button>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16">
            ...
            <div className="space-y-6 order-1 lg:order-2">
              <div className="bg-muted/30 rounded-[2rem] p-6 sm:p-8 border border-border relative overflow-hidden group">
                ...
                {order.status === SwapStatus.AWAITING_DEPOSIT && (
                  <div className="space-y-8 animate-in zoom-in duration-500">
                    <div className="flex justify-center">
                      <div className="p-4 bg-white rounded-[2rem] shadow-2xl border border-border group-hover:scale-105 transition-transform duration-500">
                    ...
                    <div className="space-y-4">
                      ...
                      <button onClick={() => { navigator.clipboard.writeText(order.depositAddress); alert('Copied to clipboard'); }} className="w-full py-4 bg-muted hover:bg-border text-foreground text-[10px] font-black uppercase tracking-widest rounded-xl border border-border transition-all active:scale-95">
                        Copy Address
                      </button>
                    </div>
                  </div>
                ...
                {order.status !== SwapStatus.AWAITING_DEPOSIT && order.status !== SwapStatus.COMPLETED && (
                  <div className="py-12 text-center animate-in zoom-in duration-500">
                    <div className="w-20 h-20 bg-blue-600/10 rounded-full flex items-center justify-center mx-auto mb-6 border border-blue-600/20">
                      <svg className="w-10 h-10 text-blue-500 animate-pulse" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>
                    </div>
                ...
                {order.status === SwapStatus.COMPLETED && (
                  <div className="py-12 text-center animate-in zoom-in duration-500">
                    ...
                    <p className="text-sm font-black text-foreground uppercase tracking-tighter">Exchange Complete</p>
                    <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest mt-2">Payout successfully broadcast to ledger.</p>
                  </div>
                )}
              </div>
